    ...
    * 10-13 * * * * : 매시간 10, 11, 12, 13분 마다
    ...
    public void checkFiles(){
        ...
        List<Path> fileListPaths = oldFileList.stream()
                ...
                .collect(Collectors.toList());    // .collect(Collectors.toList())가 정보를 List로 변환해줌

//         이전 썸네일 파일들의 전체 경로를 fileListPaths에 추가해준다.
        oldFileList.stream().map(fileDto -> Paths.get(fileDir, fileDto.getFileUploadPath(),"th_" + fileDto.getFileUuid() + "_" + fileDto.getFileName()))
...
//        파일객체는 폴더 안에 있는 모든 파일 목록을 불러오는 기능이 있다. listFiles()
        File[] files = directory.listFiles(file -> !fileListPaths.contains(file.toPath()));  // 리스트에 포함되지 않는 파일만 리스트로 저장
...
//    1일 전의 파일 경로를 만들어주는 메소드
    private String getUploadPathOldFile(){
        ...
        yesterday.add(Calendar.DATE,-1);    // DATE에서 -1하는 것임, Month로 바꾸면 달에서 -1해줌
        return sdf.format(yesterday.getTime());      // .getTime()은 Calendar타입을 DATE타입으로 바꿔줌
